...
public class WebHookController{
    ...
    public ResponseEntity receivedFromAgent(@RequestParam("file") MultipartFile file,
                                            @PathVariable("UUID") String UUID) throws IOException {
        template.convertAndSend(RabbitMQConfig.exchange,
                RabbitMQConfig.receiver, ReceiverDTO.builder().UUID(UUID).file(file.getBytes()).build());
